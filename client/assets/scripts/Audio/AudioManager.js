...
let AudioManager = cc.Class({
    ...
    // “中枪”
    playHumanZQiang: function (sex = 1) {
        var path = "resources/sounds/man"/* + (sex == 1 ? "man" : "woman")*/;
    ...
    playPokerClick: function () {
        let url = cc.url.raw('resources/sounds/public/click_wx_login.mp3');
        this._playSFX(url);
    },

    // 播放全垒打音效
    playHomeRun: function (sex = 1) {
        let path = this._soundsHumanPath(sex) + "special1.mp3";
        let url = cc.url.raw(path);
        this._playSFX(url);
    },
    ...
    // 播放发牌音效
    playFaPai: function() {
        let path1 = this._soundsHumanPath(1) + "room_start_compare.mp3";
        let url1 = cc.url.raw(path1);
        this._playSFX(url1);
    ...
    playCardType: function (sex = 1, type = -1) {
        ...
        if ((type >= 10) || (type < 0)) {
            // 10 及以上是特殊牌
            let path = this._soundsHumanPath(sex) + (type) + ".mp3";
            let url = cc.url.raw(path);
            this._playSFX(url);
            return;
        }

        let path = this._soundsHumanPath(sex) + "common" + (type + 1) + ".mp3";
        let url = cc.url.raw(path);
        this._playSFX(url);
    },
    ...
    playStartCompare: function (sex = 1) {
        let path = this._soundsHumanPath(sex) + "start_compare.mp3";
        let url = cc.url.raw(path);
        this._playSFX(url);
    },

    playWin: function () {
        let url = cc.url.raw('resources/sounds/openface/win.mp3');
        this._playSFX(url);
    },

    playLose: function () {
        let url = cc.url.raw('resources/sounds/openface/lose.mp3');
        this._playSFX(url);
    },

    coinIncome: function () {
        let url = cc.url.raw('resources/sounds/openface/coin_income.mp3');
        this._playSFX(url);
    },

    _soundsHumanPath: function (sex) {
        let path = "resources/sounds/man/"/* + (sex == 1 ? "man" : "woman") + "/"*/;
        return path;
    },
    ...
    playChatAudio:function(sex, str) {
        ...
        let number = "CHAT"+(index+1);
        let path = "resources/sounds/chat/" + number + ".mp3";
        let url = cc.url.raw(path);
        this._playSFX(url);
    },
