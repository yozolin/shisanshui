...
if($uid){           //大局数据
    ...
    if( !$_SESSION['inviteCode'] ){
        $query = "SELECT * FROM {$table_name}  ORDER BY lastLoginTime DESC  limit {$start},{$limit}";
    }else{
        $inviteCode = $_SESSION['inviteCode'];
        $query = "SELECT * FROM {$table_name} WHERE inviteCode = '{$inviteCode}' ORDER BY lastLoginTime DESC  limit {$start},{$limit}";
    }
...
<table class="table table-striped table-bordered table-hover dataTables-example dataTable">
    ...
    <?php }}else if($uid ){foreach($data as $key=>$value){  ?>
        ...
        <?php if(!$pid){?>
            ...
            <td><?php  echo count(json_decode($value['users']));
                ?></td>
            ...
            <?php $count = count(json_decode($value['users']));?>
            <td><?php for($i=0;$i<$count;$i++){echo json_decode($value['users'])[$i]."<br/>";}?></td>
            
            
            <td><?php for($i=0;$i<$count;$i++){echo json_decode($value['scores'])[$i]."<br/>";}?></td>
            <td><?php for($i=0;$i<$count;$i++){echo json_decode($value['playersInfo'])->players[$i]->nickname."<br/>";}?></td>
        ...
        <?php }else{?>
                ...
                <?php $con = count(json_decode($value['info1']));?>
            <td><?php echo $value['info'];?></td>
            <td><?php echo "第".$value['gameIndex']."局"?></td>
            <td><?php echo $value['score']?></td>
            <td><?php for($i=0;$i<$con;$i++){echo json_decode($value['info1'])[$i]."<br/>";}?></td>
