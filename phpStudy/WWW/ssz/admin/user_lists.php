...
if($_SESSION['type']=="admin"){
    $id = $_SESSION['id'];
    $adInfo = select_user("admin",$id);
}else{
    $id = $_SESSION['id'];
    $adInfo = select_user("agent",$id);
}
?>	
...
<table class="table table-striped table-bordered table-hover dataTables-example dataTable">
    <thead>
        <tr>
            ...
            <th width="">房卡数</th>
            <th width="">状态</th>
            <?php if($_SESSION['level']=='127'||$adInfo['permission']==1){?>
                <th width="">比牌设置</th>
            <?php }?>
            <th width="">ip地址</th>
            <th width="">上次登录时间</th>
    ...
    <?php foreach($data as $key=>$value){ ?>
        <tr>
            ...
            <?php if($_SESSION['level']=='127'||$adInfo['permission']==1){?>
                        <?php 
                            $paiInfo = json_decode($value['hong'],true);
                     ...
                     ?>
                <td> <?php $arr = $arrs2;if(is_array($arr)){foreach ($arr as $values)echo $values.'&nbsp;&nbsp;';}else{echo $arr;}?>
                 <a href="user_haopai.php?id=<?php echo $value['id'] ?>">设置</a>
                 </td>
            <?php }?>
